...
for(i in 1: nrow(XYpos)) {
  ...
  if(length(polyCrop) > 0) { # rasterize shapefile if polygons exist 
    ...
    flagOrtho  <- length(which(is.na(values(uavCrop[[1]])) == TRUE)) == 0 # TRUE if no NA in crop
  } else {
  ...
  if(flagOrtho && flagPolyNA) {
    ...
    filename  <- paste(site, tileTag, paste0("b", nlayers(uav)), sep = "_")
    image_write(uavCrop, format = "png",
